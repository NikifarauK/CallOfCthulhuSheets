...
namespace CallOfCthulhuSheets.ViewModels
{
    ...
    public class SkillPointsViewModel : BaseViewModel
    {
        ...
        private async Task RefreshOccupation(string id)
        {
            ...
            var allSkills = SqliteRepo.GetItemsAsync<Skill>();
        ...
        private async Task SkillTypeSelected()
        {
            ...
            switch (name)
            {
                ...
                case "новый":
                    ...
                    chosenSkill = await Shell.Current.Navigation.NavigationStack.LastOrDefault()?.ShowPopupAsync(
                    ...
                    SqliteRepo.AddItemAsync(chosenSkill);
                    break;
                default:
